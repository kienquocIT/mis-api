...
from ..sales.saleorder.utils.logical_after_finish import AfterFinishHandler
...
def reset_and_run_reports_sale(run_type=0):
    if run_type == 0:  # run report revenue, customer, product
        ...
        for sale_order in SaleOrder.objects.filter(system_status__in=[2, 3]):
            AfterFinishHandler.push_to_report_revenue(sale_order)
            AfterFinishHandler.push_to_report_product(sale_order)
            AfterFinishHandler.push_to_report_customer(sale_order)
    if run_type == 1:  # run report cashflow
        ReportCashflow.objects.all().delete()
        for sale_order in SaleOrder.objects.filter(system_status__in=[2, 3]):
            AfterFinishHandler.push_to_report_cashflow(sale_order)
...
def reset_set_product_transaction_information():
    ...
    # output
    for so in SaleOrder.objects.filter(system_status__in=[2, 3]):
        AfterFinishHandler.update_product_wait_delivery_amount(instance=so)
