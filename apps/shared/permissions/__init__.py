...
BASTION_FROM_TYPE = Literal['opp', 'prj']
...
class PermissionController:
    ...
    @staticmethod
    def _valid_allow_option(item, app_data_by_id, bastion_from: BASTION_FROM_TYPE = None):
        app_id = item['app_id']
    ...
    def valid_config(self, config_data, plan_id_arr: list = None, app_id_arr: list = None, checked_data: list = None):
        if not plan_id_arr:
            plan_id_arr = []
        if not app_id_arr:
            app_id_arr = []
    ...
    def push_range_to_key(
            ...
            config_by_id=None,
            data_of_range=None,
            permit_from: Literal['general', 'ids', 'opp', 'prj'] = None,
    ):
        """
        ...
        if allow_range_or_id not in data_getter:
            ...
            for key, value in data_of_range.items():
                data_getter[allow_range_or_id][key] = value
