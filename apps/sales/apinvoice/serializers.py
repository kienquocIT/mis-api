...
class APInvoiceCreateSerializer(AbstractCreateSerializerModel):
    ...
    def create(self, validated_data):
        ...
        ap_invoice_obj = APInvoice.objects.create(**validated_data)

        APInvoiceCommonFunc.create_goods_receipt_mapped(ap_invoice_obj, goods_receipt_mapped_list)
        APInvoiceCommonFunc.create_items_mapped(ap_invoice_obj, data_item_list)
        APInvoiceCommonFunc.create_files_mapped(ap_invoice_obj, attachment_list)

        return ap_invoice_obj
...
class APInvoiceDetailSerializer(AbstractDetailSerializerModel):
    ...
    def get_attachment(cls, obj):
        return [item.attachment.get_detail() for item in obj.ap_invoice_attachments.all()]
...
class APInvoiceUpdateSerializer(AbstractCreateSerializerModel):
    ...
    def update(self, instance, validated_data):
        ...
        APInvoiceCommonFunc.create_items_mapped(instance, data_item_list)
        APInvoiceCommonFunc.create_files_mapped(instance, attachment_list)
...
class APInvoiceCommonFunc:
    @staticmethod
    def create_goods_receipt_mapped(ap_invoice_obj, gr_mapped_list):
        bulk_data = []
        for item in gr_mapped_list:
            bulk_data.append(APInvoiceGoodsReceipt(ap_invoice=ap_invoice_obj, **item))
        APInvoiceGoodsReceipt.objects.filter(ap_invoice=ap_invoice_obj).delete()
        APInvoiceGoodsReceipt.objects.bulk_create(bulk_data)
        return True

    @staticmethod
    def create_items_mapped(ap_invoice_obj, data_item_list):
        bulk_data = []
        sum_pretax_value = 0
        sum_tax_value = 0
        ...
        APInvoiceItems.objects.filter(ap_invoice=ap_invoice_obj).delete()
        APInvoiceItems.objects.bulk_create(bulk_data)
        ap_invoice_obj.sum_pretax_value = sum_pretax_value
        ap_invoice_obj.sum_tax_value = sum_tax_value
        ap_invoice_obj.sum_after_tax_value = sum_after_tax_value
        ap_invoice_obj.save(
    ...
    @staticmethod
    def create_files_mapped(ap_invoice_obj, attachment_list):
        SerializerCommonHandle.handle_attach_file(
            relate_app=Application.objects.filter(id=APInvoice.get_app_id()).first(),
            model_cls=APInvoiceAttachmentFile,
            instance=ap_invoice_obj,
