# Generated by Django 4.2.8 on 2025-04-02 17:09

from django.db import migrations, models
import django.db.models.deletion
import django.utils.timezone
import uuid


class Migration(migrations.Migration):

    dependencies = [
        ('company', '0018_delete_companybankaccount'),
        ('hr', '0004_employee_email_app_password_and_more'),
        ('tenant', '0001_initial'),
        ('saledata', '0064_remove_account_parent_account_and_more'),
        ('purchasing', '0015_alter_purchaseorderproduct_gr_remain_quantity_and_more'),
    ]

    operations = [
        migrations.RemoveField(
            model_name='purchaseorderpaymentstage',
            name='payment_ratio',
        ),
        migrations.RemoveField(
            model_name='purchaseorderpaymentstage',
            name='value_after_tax',
        ),
        migrations.AddField(
            model_name='purchaseorder',
            name='purchase_order_invoice',
            field=models.JSONField(default=list, help_text='read data invoice, use for get list or detail purchase order'),
        ),
        migrations.AddField(
            model_name='purchaseorderpaymentstage',
            name='cash_inflow_done',
            field=models.BooleanField(default=False),
        ),
        migrations.AddField(
            model_name='purchaseorderpaymentstage',
            name='date',
            field=models.DateTimeField(null=True),
        ),
        migrations.AddField(
            model_name='purchaseorderpaymentstage',
            name='date_type',
            field=models.CharField(blank=True, max_length=200),
        ),
        migrations.AddField(
            model_name='purchaseorderpaymentstage',
            name='invoice',
            field=models.IntegerField(null=True),
        ),
        migrations.AddField(
            model_name='purchaseorderpaymentstage',
            name='invoice_data',
            field=models.JSONField(default=dict, help_text='data json of invoice'),
        ),
        migrations.AddField(
            model_name='purchaseorderpaymentstage',
            name='is_ar_invoice',
            field=models.BooleanField(default=False),
        ),
        migrations.AddField(
            model_name='purchaseorderpaymentstage',
            name='ratio',
            field=models.FloatField(null=True),
        ),
        migrations.AddField(
            model_name='purchaseorderpaymentstage',
            name='reconcile_data',
            field=models.JSONField(default=list, help_text='data json of reconcile'),
        ),
        migrations.AddField(
            model_name='purchaseorderpaymentstage',
            name='tax_data',
            field=models.JSONField(default=dict, help_text='data json of tax'),
        ),
        migrations.AddField(
            model_name='purchaseorderpaymentstage',
            name='value_reconcile',
            field=models.FloatField(default=0),
        ),
        migrations.AddField(
            model_name='purchaseorderpaymentstage',
            name='value_tax',
            field=models.FloatField(default=0),
        ),
        migrations.AddField(
            model_name='purchaseorderpaymentstage',
            name='value_total',
            field=models.FloatField(default=0),
        ),
        migrations.CreateModel(
            name='PurchaseOrderInvoice',
            fields=[
                ('id', models.UUIDField(default=uuid.uuid4, editable=False, primary_key=True, serialize=False)),
                ('title', models.CharField(blank=True, max_length=100)),
                ('code', models.CharField(blank=True, max_length=100)),
                ('date_created', models.DateTimeField(default=django.utils.timezone.now, editable=False, help_text='The record created at value')),
                ('date_modified', models.DateTimeField(auto_now=True, help_text='Date modified this record in last')),
                ('is_active', models.BooleanField(default=True)),
                ('is_delete', models.BooleanField(default=False)),
                ('remark', models.CharField(blank=True, max_length=500, null=True, verbose_name='remark')),
                ('date', models.DateTimeField(null=True)),
                ('ratio', models.FloatField(null=True)),
                ('tax_data', models.JSONField(default=dict, help_text='data json of tax')),
                ('total', models.FloatField(default=0)),
                ('balance', models.FloatField(default=0)),
                ('order', models.IntegerField(default=1)),
                ('company', models.ForeignKey(help_text='The company claims that this record belongs to them', null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='%(app_label)s_%(class)s_belong_to_company', to='company.company')),
                ('employee_created', models.ForeignKey(help_text='Employee created this record', null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='%(app_label)s_%(class)s_employee_creator', to='hr.employee')),
                ('employee_modified', models.ForeignKey(help_text='Employee modified this record in last', null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='%(app_label)s_%(class)s_employee_modifier', to='hr.employee')),
                ('purchase_order', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='purchase_order_invoice_purchase_order', to='purchasing.purchaseorder', verbose_name='purchase order')),
                ('tax', models.ForeignKey(null=True, on_delete=django.db.models.deletion.CASCADE, related_name='purchase_order_invoice_tax', to='saledata.tax', verbose_name='tax')),
                ('tenant', models.ForeignKey(help_text='The tenant claims that this record belongs to them', null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='%(app_label)s_%(class)s_belong_to_tenant', to='tenant.tenant')),
            ],
            options={
                'verbose_name': 'Purchase Order Invoice',
                'verbose_name_plural': 'Purchase Order Invoices',
                'ordering': ('order',),
                'permissions': (),
                'default_permissions': (),
            },
        ),
    ]
