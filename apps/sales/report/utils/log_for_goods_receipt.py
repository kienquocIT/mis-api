...
class IRForGoodsReceiptHandler:
    ...
    @classmethod
    def gr_has_pr(cls, instance, doc_data, all_lot, all_serial):
        for gr_item in instance.goods_receipt_product_goods_receipt.all():
    ...
    def push_to_inventory_report(cls, instance):
        ...
        all_lot = cls.get_all_lot(instance)
        all_serial = cls.get_all_serial(instance)
        doc_data = []
        if instance.goods_receipt_type in [0, 1, 3]:  # GR by PO/IA
            if instance.goods_receipt_pr_goods_receipt.count() == 0:
                doc_data = cls.gr_has_no_pr(instance, doc_data, all_lot, all_serial)
            else:
                doc_data = cls.gr_has_pr(instance, doc_data, all_lot, all_serial)
