...
from apps.core.base.models import ApplicationProperty, Application
...
class AdvanceFilterCreateSerializer(serializers.ModelSerializer):
    filter_condition = serializers.JSONField(required=False)
    ...
    def validate_application(cls, value):
        if value:
            try:
                ...
                raise serializers.ValidationError({'Application': BaseMsg.NOT_EXIST})
        raise serializers.ValidationError({'Application': BaseMsg.REQUIRED})

    @classmethod
    def validate_filter_condition(cls, value):
        if not value:
            raise serializers.ValidationError({'Filter Condition': BaseMsg.REQUIRED})
        return value
...
class AdvanceFilterUpdateSerializer(serializers.ModelSerializer):
    filter_condition = serializers.JSONField(required=False)
    application = serializers.UUIDField(required=False)
...
class AdvanceFilterListSerializer(serializers.ModelSerializer):
    class Meta:
        model = List
        fields = (
            'id',
...
class AdvanceFilterDetailSerializer(serializers.ModelSerializer):
    class Meta:
        model = List
        fields = (
            'id',
